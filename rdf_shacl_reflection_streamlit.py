...
"""
            ...
            with st.spinner("🔄 Generating initial RDF & SHACL..."):
                rdf_1, shacl_1 = generate_rdf_shacl(data_input, system_prompt, model_info)

            st.subheader("🟢 Initial RDF Output")
            st.code(rdf_1, language="turtle")
            st.subheader("🟢 Initial SHACL Output")
            st.code(shacl_1, language="turtle")

            explanation_1 = get_correction_explanation(rdf_1, shacl_1, model_info)
            st.markdown("### 🧠 Why Improve This (Initial)")
            st.info(explanation_1)

            ### 2️⃣ FIRST OPTIMIZATION
            with st.spinner("🔄 Optimizing RDF/SHACL (Step 1)..."):
                rdf_2, shacl_2 = generate_rdf_shacl(
                    data_input, system_prompt, model_info,
                    previous_output=f"{rdf_1}\n\n{shacl_1}\n\nCritique:\n{explanation_1}"
                )

            st.subheader("🟡 Optimized RDF v1")
            st.code(rdf_2, language="turtle")
            st.subheader("🟡 Optimized SHACL v1")
            st.code(shacl_2, language="turtle")

            explanation_2 = get_correction_explanation(rdf_2, shacl_2, model_info)
            st.markdown("### 🧠 Why Improve This (After Optimization 1)")
            st.info(explanation_2)

            ### 3️⃣ FINAL OPTIMIZATION
            with st.spinner("🔄 Final optimization of RDF/SHACL..."):
                rdf_3, shacl_3 = generate_rdf_shacl(
                    data_input, system_prompt, model_info,
                    previous_output=f"{rdf_2}\n\n{shacl_2}\n\nCritique:\n{explanation_2}"
                )

            st.subheader("🔵 Final RDF v2 (Validated)")
            st.code(rdf_3, language="turtle")
            st.download_button("⬇️ Download Final RDF", rdf_3, file_name="final_mechanical_test.ttl")

            st.subheader("🔵 Final SHACL v2 (Validated)")
            st.code(shacl_3, language="turtle")
            st.download_button("⬇️ Download Final SHACL", shacl_3, file_name="final_mechanical_test_shape.ttl")
